...
> **Execution:** `/flow-fast` implements this plan. Mark a task's own checkbox when
...
Eight tasks, dependency order. `design.md`'s **Decisions** is canonical for every decision a task
implements; a task names the decision it implements and what the edit must say, never a second
copy of it. The staging note (`docs/superpowers/research/kan-492.md` and its `kan-492/` plan
directory) was already deleted by the brainstorm phase's own create-artifacts commit, per
`skills/flow-fast/brainstorm.md` section C — no task here repeats that deletion. Task 1 is the one
edit to a `/flow` file; tasks 2–6 rewrite `skills/flow-fast/`; task 7 edits the contracts; task 8
the command stubs.
...
**Chained stage marks, every phase file (tasks 2–5):** each stage's `flow stage end` and the next
...
- [x] 1. `skills/flow/panel-dispatch.md` — the shared dispatch file

Per `design.md`'s **Review panel** decision. New file holding, moved verbatim out of
...
- [x] 2. `skills/flow-fast/review.md` — panel-dispatch citation, narrower re-run, literal handoff

Per `design.md`'s **Review panel** and **Handoff** decisions. Rewrite: cite
...
- [x] 3. `skills/flow-fast/brainstorm.md` — direct write, minimal artifacts

Per `design.md`'s **Brainstorm** and **Artifacts** decisions and its spec-fix note. Section B: read
...
- [x] 4. `skills/flow-fast/implement.md` — chained marks, minimal fields

Per `design.md`'s **Stage marks** and **Artifacts** decisions. The task loop reads `**Files:**`,
...
- [x] 5. `skills/flow-fast/finish.md` — the one-run finish

Per `design.md`'s **Finish**, **Cleanup**, **Jira** and **Stage marks** decisions. Rewrite as
`/flow-fast`'s own procedure, merge-and-push only: the three guards
(`check-finish-preflight.sh`, `check-unfinished-work.sh`, `check-base-moved.sh`, one chained
...
- [x] 6. `skills/flow-fast/SKILL.md` and its guard symlinks

Per `design.md`'s **Finish** decision and its stage-keys/guard-set consequences. Stage-key table:
one `finish.md` row carrying all twelve finish keys (no run 1 / run 2 split). Guard set:
`check-archive-scope.sh` joins, `check-cleanup-complete.sh` leaves — the presence-check sentence,
the **Guard set** list and the dropped-guard list all move together. Guardrails: never ask the
landing question, never read `## default landing route`, never push the change branch, never open
a PR — merge-and-push is the only route. Model resolution unchanged. Symlinks: add
`skills/flow-fast/scripts/check-archive-scope.sh`, remove
`skills/flow-fast/scripts/check-cleanup-complete.sh` — plus `prepare-archive-branch.sh` and
`resolve-base-branch.sh`, which task 5's `finish.md` invokes directly and
`check-guard-symlinks.sh` therefore requires present alongside the other guards this task adds.

  - [ ] **Step 1: Edit** `skills/flow-fast/SKILL.md` per the content above.
  - [ ] **Step 2: Symlinks** — `ln -s ../../../scripts/check-archive-scope.sh`,
    `ln -s ../../../scripts/prepare-archive-branch.sh`, `ln -s
    ../../../scripts/resolve-base-branch.sh` into `skills/flow-fast/scripts/`; `git rm` the
    `check-cleanup-complete.sh` link there.
  - [ ] **Step 3: Verify** — `scripts/check-vocabulary.sh`, `scripts/check-references.sh`,
    `scripts/check-markdown-integrity.py`, `scripts/check-contract-budget.sh`,
...
`skills/flow-fast/scripts/prepare-archive-branch.sh`,
`skills/flow-fast/scripts/resolve-base-branch.sh`
...
- [x] 7. Contracts — finish contracts, Jira transitions, artifacts registry

Per `design.md`'s **Spec fixes** and **Jira** decisions. `finish-contract-run1.md`: the
artifact-copy skip's `/myflow-fast` sentence becomes `/flow-fast`. `finish-contract-run2.md`: the
check-4 `/myflow-fast` override paragraph is deleted (`/flow-fast` skips checks 1–4 outright and
runs no run 2); the step-7 and step-9 `/flow-fast` exemption sentences are deleted for the same
reason. `jira-integration.md` **Transitions**: a `/flow-fast` row — `In Progress` at kickoff,
`Done` after the push. `artifacts-registry.md`: the *Remote branch*, *Archive branch* and
*Rendered ledger and panel record* rows each gain the clause that `/flow-fast` creates none.
...
- [x] 8. `commands/flow-fast.md` and `commands-claude/flow-fast.md`

Per `design.md`'s **Finish** and **Brainstorm** decisions. Both stubs describe the command as it now
