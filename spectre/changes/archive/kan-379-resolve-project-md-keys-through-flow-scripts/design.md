...
```json verified:matches stateResolveOutput in stats/cmd/flow/state.go
...
```bash verified:scripts/check-model-resolution-shell.sh passes
...
| Set | Before | After |
|---|---|---|
| A — creating-run parent | 282272 bytes, ~70568 tok | 207348 bytes, ~51837 tok |
| B — per-subagent fixed overhead | 25694 bytes, ~6423 tok | unchanged by this design |
| C — planner | 97362 bytes, ~24340 tok | 78173 bytes, ~19543 tok |
