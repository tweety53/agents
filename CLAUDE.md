...
deliberately **not** kept byte-identical. Expect both renderings in one session: `/myflow-info`
reads the canonical block live and prints what it finds. A difference in what the three lines *say*
is drift worth reporting; a difference in how the third one is phrased is not. Everything else is
cited rather than copied: the state diagram and the per-command stage table live under
