...
```text verified:run on 2026-08-17 against the live global install
...
**Status:** active
**Chosen:** Every command skill that actually invokes a guard — which turned out to be three:
`myflow-do`, `myflow-finish` and `myflow-fast`. Each carries symlinks for exactly the guards it can
invoke, plus `lib` where one of those guards sources it.

**Corrected during implementation.** This decision originally named five skills, including
`myflow-start` and `myflow-status`. Both turned out to invoke no guard at all: `/myflow-start` runs
*the project's configured* provenance and build-green guards, resolved through `.myflow/project.md`,
and `/myflow-status` deliberately reimplements `check-finish-preflight.sh`'s merge-status steps in
prose rather than running it — its own text says so in three places. The original map counted those
prose mentions as invocations, because it was derived by grepping for guard names before the
invoking-vs-prose boundary existed. A directory nothing resolves against goes stale unnoticed, which
is the same reason `skills/myflow-contracts/` gets none, so both were dropped rather than kept with a
rationale invented for them.
...
```text unverified:the layout this change creates; the directories do not exist yet
...
```text verified:read from the live global install on 2026-08-17
...
```text verified:printed by the scratch-tree run described above, on 2026-08-17
...
| `myflow-fast` | the union of `myflow-do`'s and `myflow-finish`'s sets |
| `myflow-start` | *(none — it invokes no guard of its own)* |
| `myflow-status` | *(none — same reason; see below)* |
...
```text unverified:the shape the operator chose during brainstorming; no run emits it yet
