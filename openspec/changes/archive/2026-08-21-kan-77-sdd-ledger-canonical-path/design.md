# Design — kan-77-sdd-ledger-canonical-path
...
### Artifact brevity lives in `pipeline.md`, not in each skill

**ID:** brevity-in-pipeline
**Status:** active
**Chosen:** one section in `skills/myflow-contracts/pipeline.md` — the one file every `/myflow-*`
command loads before any other step, so a single statement reaches every run.
**Considered:**
- *A copy in each artifact-writing skill* (`myflow-start`, `myflow-do`, `myflow-finish`) — ruled out:
  four copies drift, and whichever skill lacked one would silently exempt its own artifacts.
- *Amending the global be-brief rule's carve-out* — ruled out for this change: it governs every
  project and every subagent, far past myflow, and the operator asked for myflow runs. The section
  narrows that carve-out for myflow artifacts instead.
- *A length guard or byte budget per artifact* — ruled out and forbidden by the spec: a budget on a
  per-change artifact rewards dropping the facts the rule requires kept.

### The folded-red-task guard gap is fixed here, not deferred

**ID:** fix-folded-red-guard
**Status:** active
**Chosen:** repair `check-task-commit-fields.py` in this change (tasks 7–8). It parses
`**Squash-with:**` only to terminate a preceding field and acts on it nowhere, so a `Build: red` task
can never pass — its commit is folded into its partner's by design and its declared subject ceases to
exist. Found by this change's own `f0905f2`; `377b7dd` (kan-102) is a prior instance.
**Considered:**
- *File a follow-up, check by hand this run* — keeps KAN-77 on one subject, but leaves the guard
  broken for every folded red task until that ticket lands, and this change produces such a commit.
- *Un-fold into two commits* — the guard would pass unchanged, but a red commit would stand on the
  branch, which is exactly what `Build: red` + `Squash-with:` exists to prevent, and it contradicts
  the kan-102 precedent.
...
### 6. The folded-red-task guard

`check-task-commit-fields.py` gains one resolution step: when the task named on the command line is
tagged `**Build:** red`, resolve its `**Squash-with:**` partner in the same plan and check against
the partner's `**Commit:**` subject with a `**Files:**` set that is the union of both. The same
resolution applies when invoked for the partner, so either id gives one verdict. A `Squash-with:`
naming a missing task fails; a partner itself tagged `red` fails. A task with no `Squash-with:` is
untouched — the file set is not widened for ordinary tasks.

### 7. Artifact brevity

One section in `skills/myflow-contracts/pipeline.md`, beside **Handoff output** — nearest neighbour
by subject, what a run writes for a human. It states: which artifacts it binds; that brevity never
withholds a fact; the never-compress list (guard-parsed plan fields, provenance tags, decision
`ID:`/`Status:` lines, normative statements and scenarios, the panel record's marker blocks); that it
narrows the be-brief rule's "docs and specs stay full" carve-out for those files only; and that no
