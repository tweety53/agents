# Design — cut per-command load further

Source: `docs/superpowers/specs/2026-08-07-kan-87-cut-per-command-load-further-design.md`, approved
at the design gate on 2026-08-07. Adapted here, not duplicated: where the two differ, the delta
specs under `specs/` are normative and this file is the how.

**Jira:** KAN-87. **Follows:** KAN-82, which cut a `/myflow-start` run from 156,109 to 103,844
bytes.
...
Measured on this tree: 1,348 lines, 88,253 bytes, **30,351 tokens**.
<!-- measured: Read tool on skills/myflow-contracts/pipeline.md @ 63fa6ee -->
It is read in two pages because it exceeds the tool's per-read cap. Every `/myflow-*` command loads
it.
...
| Every `Finish contract` citation still reaches what it promises | Each read individually; `myflow-do`'s is the one the hoist exists for. **Re-run the `grep` as the last action and read what it prints** — a count fixed at design time goes stale the moment a later task adds a citation, which is what happened here |
...
and is closed by reading, which is why the plan names each citation individually — and re-runs the
`grep` as its last action rather than trusting a count fixed at design time. That count was written
as four and became five when a later fix added `myflow-info`'s pointer line; the enumeration is the
command's output, never a substitute for running it.
