...
### The apply worktree is git-ignored under `.worktrees/`, not moved to a sibling directory

**ID:** worktrees-dir-gitignored
**Status:** active
**Chosen:** add `.worktrees/` to `.gitignore`. This change's own apply worktree lives at
`/Users/tweety53/Projects/agents/.worktrees/kan-107-remove-manual-test-guide-and-gate`, and
`superpowers:using-git-worktrees` requires the worktree directory be git-ignored before a worktree
is created there; `.worktrees/` was chosen as that directory, so the entry is a prerequisite of
using the worktree at all, not a subject of this change.
**Considered:**
- *Move the worktree to a sibling directory instead* — ruled out: every path already recorded in
  the state file, the SDD ledger and the published handoffs names the current location, and moving
  it now would orphan those references for no benefit.
