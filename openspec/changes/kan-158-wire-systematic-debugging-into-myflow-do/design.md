...
context from the parent session. That global rule only reaches a dispatched subagent if the dispatch
prompt names it explicitly — the same reason the implementer dispatch already carries an explicit
TDD bullet rather than relying on a subagent to remember the global rule on its own.
