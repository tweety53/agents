...
**Measured:** `lsof -d cwd -Fpn` returns 283 cwd rows in a single pass on this machine, no `sudo`,
emitting `p<pid>` / `f cwd` / `n<path>` triples. The count is point-in-time — it is however many
processes happen to be running — and was 290 when this decision was first written.
<!-- measured: lsof -d cwd -Fpn 2>/dev/null | grep -c '^n' @ /Users/tweety53/Projects/agents, 2026-08-22 — 290 when written, 283 on re-measurement the same day during implementation; varies with what is running -->
**Considered:**
