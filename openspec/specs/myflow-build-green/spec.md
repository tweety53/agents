# myflow-build-green Specification

## Purpose
TBD - created by archiving change kan-13-myflow-planning-and-status-fixes. Update Purpose after archive.
## Requirements
...
`**Build:** red — merges with Task <N>`, where `<N>` names one or more other tasks in the same
plan that this task is dispatched together with as a single unit.
...
leave a green build, and SHALL name at least one merge partner.
...
- **THEN** the plan tags it `**Build:** red — merges with Task <N>`, naming the task(s) it is
  dispatched with as one unit
...
task is tagged `red` with no named merge partner; a named merge partner does not exist in the
plan, or is not itself tagged `green`; or a `red` chain never resolves to `green` before the
plan's last task.
...
- **WHEN** the build-green guard runs against a `tasks.md` where a task is tagged
  `**Build:** red — merges with Task <N>` and Task `<N>` does not exist, or is itself tagged `red`
- **THEN** the guard fails, naming the unresolved task

#### Scenario: A resolved red chain passes

- **WHEN** the build-green guard runs against a `tasks.md` where every `red`-tagged task names a
  merge partner that exists and is tagged `green`
