# myflow-stats-build Specification

## Purpose
What the stats project's build target owes an operator: every binary the documentation tells them to
run, written to the path it names, refreshed by the one command it names — and every other package
still compiled, including the main packages the target emits no binary for.
