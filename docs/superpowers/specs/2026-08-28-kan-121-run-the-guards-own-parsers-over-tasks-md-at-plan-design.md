...
**Change:** `kan-121-run-the-guards-own-parsers-over-tasks-md-at-plan`

## Problem

A `tasks.md` that a downstream guard cannot parse is written, dispatched, and rejected only
**after** work has been done against it, forcing a mid-run reshape. `check-task-build-green.sh`
already runs at plan time — it is a `## lint` step and scans a whole `tasks.md` in one pass.
`check-task-commit-fields.sh` cannot: its signature is
`<worktree> <task-id> <commit-sha> [parent-sha]`, so there is no way to run its field parser over
a plan before a commit exists. Every defect below is invisible until that commit lands.
...
`spectre/changes/` currently holds two changes the store reports `FINISHED` but which were never
moved to `archive/` — `kan-359-multi-glob-ui-paths-matches-nothing` and
`panel-roster-follows-the-settings-list`. The new lint step's no-argument scan covers both. They
must come back clean, or their plans are fixed as part of this change.
