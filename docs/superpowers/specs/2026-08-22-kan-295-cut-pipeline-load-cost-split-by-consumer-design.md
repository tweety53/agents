# Cut `pipeline.md`'s load cost — split by consumer, move rationale to per-file appendices
...
requires every `/myflow-*` command to load it in full before any other step. It is the single
largest fixed context cost in the pipeline, paid identically by `/myflow-start`, `/myflow-do`,
`/myflow-finish`, `/myflow-fast` and `/myflow-status` — including commands that need a fraction of
it. Surfaced while running `/myflow-start` for KAN-242: roughly 140k tokens were spent before the
first design question, and the mandated load was the largest single contributor.
...
| Model policy | 6661 |
| Stage marks | 6630 |
| Temporary artifacts registry | 6183 |
| Change name resolution | 4256 |
| Git boundaries | 3946 |
| Handoff output | 3875 |
| Progress visibility | 2393 |
| Guard presence check | 2161 |
| State transitions | 1913 |
| Artifact brevity | 1762 |
| Guard resolution | 1527 |
| Resolving a change's worktrees | 1517 |
| Rendering the session records | 1486 |
| States | 1450 |
| Stage exit — never the command's own judgment | 817 |
| IntelliJ commands | 651 |
| Command surface | 561 |
| Wrong state for this command | 615 |
| Finish contract / State file / Project configuration / Jira integration pointers | 1165 |

<!-- measured: awk section-size pass over skills/myflow-contracts/pipeline.md @ 15f11cc -->

## Scope

A relocation and a split. **No contract's meaning changes.** No new guard, no rule-table row, no
installer code, no `/myflow-*` command behaviour change.
...
<!-- predicted: pipeline.md ~26 KB after both the split and the rationale sweep -->
...
`commands-claude/`, `CLAUDE.md`, `AGENTS.md`, `README.md` and `openspec/specs/`. Each is repointed
to whichever file now holds its section.
