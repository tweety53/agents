...
# kan-515 — mockup frame geometry, three-panel diff and size gate in compose-mockup-frames
...
Four tasks, dependency order, one repository (`agents`). No spec, no migration.
...
- [ ] 1. Geometry crop, diff panel, ratio and size gate in `compose-mockup-frames.py`
...
**Commit:** `feat(scripts): crop mockup frames, add a diff panel and a size gate to compose-mockup-frames`
  ...
  - [ ] **Step 1: Parse the optional fourth argument.** `argv[4]`, when present, is
  ...
  - [ ] **Step 2: Crop.** With geometry, after the frame PNG loads: `s = scale`, page background
  ...
  - [ ] **Step 3: Size gate.** With geometry, `cap.size != frame.size` after the crop is the
  ...
  - [ ] **Step 4: Three panels.** Sizes equal: `diff = ImageChops.difference(cap, frame).convert("L")
  ...
  - [ ] **Step 5: stdout.** Each output line becomes `<abs composite path> diff=<ratio:.4f>`, or
  ...
  - [ ] **Step 6: Harness.** Add Cases 20–25 named above to `test-compose-mockup-frames.sh` on its
...
- [ ] 2. `mockup frame` in the section guard's closed vocabulary
...
**Commit:** `feat(scripts): accept a mockup frame row in the visual verification section`
  ...
  - [ ] **Step 1: Vocabulary.** In `check_setting_row`, add `key != "mockup frame"` to the closed
  ...
  - [ ] **Step 2: Harness.** Cases 34–36 on the harness's own fixture-writing helpers, extending
...
- [ ] 3. The contract: `mockup frame`, the crop rule, the three-panel composite
...
**Commit:** `docs(flow-contracts): declare the mockup frame geometry and the three-panel composite`
  ...
  - [ ] **Step 1: Settings table.** Add the row `| \`mockup frame\` | no | \`scale=<int> status=<px>
  ...
  - [ ] **Step 2: Sidecar paragraph.** Replace "The composite … is the captured frame on the left
...
- [ ] 4. `flow.visual-verify` step 9 passes the geometry and reads three panels
...
**Commit:** `feat(flow): pass the mockup frame geometry to the compose step and read its diff panel`
  ...
  - [ ] **Step 1: The verifier dispatch.** In the prompt-contents sentence (the one naming
    "its resolved `mockups` root when declared"), add "and its `mockup frame` value when declared".
  - [ ] **Step 2: Step 9.** The compose command gains the fourth argument, quoted:
  ...
  - [ ] **Step 3: Step 10.** The `visual-verification.md` entry per composed pair also records the
    ratio.
  - [ ] **Step 4: Verify.**
