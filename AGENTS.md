...
`README.md`'s "How the pipeline works" section covers the same ground for a reader browsing the
repository directly — it is not copied by `setup.sh` into any project, so it is not cited here as a
source of record.
...
command. `/myflow-do` emits both the staged diff and the run instructions, so reviewing and
