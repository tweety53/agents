...
Also follow `rules/myflow-manual-review.mdc` (always-on) — it is a stub, so **load `skills/myflow-contracts/pipeline.md` first**; that file holds the stages, transitions, gates, boundaries and flags, and is canonical. See the Codex rule-sourcing note at the top of this file for how to reach the rule.
