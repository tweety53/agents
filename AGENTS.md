...
| `/myflow-finish <name>` | Which run happens is `scripts/check-finish-preflight.sh`'s verdict, taken once per recorded worktree — `RUN1`, `RUN2` from every worktree, or `REFUSE`, which stops and asks you rather than guessing. **Run 1 — verdict `RUN1`:** asks how it should land (open a PR *(default)*, merge and push, or handle it manually), preserves the change's session records into the repository, commits the staged work, takes that route, **stops** at `IN_PROGRESS`. **Run 2 — verdict `RUN2`:** syncs delta specs, archives (with any nested `<name>-fix-N`), **commits and pushes the archive**, **removes the worktrees and branches** after four gating safety checks plus a disclosure of the ignored files `--force` will destroy, then **removes the proposal artifact source** — only once its preserved copy exists in the repository → `state: FINISHED`. **Runs no tests, linters or coverage check** |
