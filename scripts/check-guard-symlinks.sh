...
declare_if_present "flow-fast" "invokes no guard by contract — git isolation only, the project's own lint and test commands through project-get.sh, and the flow CLI; no guard script runs in a /flow-fast run"
