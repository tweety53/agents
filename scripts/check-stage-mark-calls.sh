...
stage_value() {
  local text="$1"
  printf '%s\n' "$text" | grep -oE -- "-stage[[:space:]]+('[^']*'|\"[^\"]*\"|[^[:space:]]+)" | head -n1 \
    | sed -E "s/^-stage[[:space:]]+//; s/^'(.*)'\$/\1/; s/^\"(.*)\"\$/\1/"
}
...
assemble_calls() {
  ...
  awk '
    ...
    function stripcr(s) { sub(/\r$/, "", s); return s }
    {
      ...
      verb = ""
      if (line ~ /^[[:space:]]*flow stage begin([[:space:]]|\\|$)/) {
        verb = "flow stage begin"
      } else if (line ~ /^[[:space:]]*flow record dispatch([[:space:]]|\\|$)/) {
        verb = "flow record dispatch"
      }
      if (verb != "") {
        if (line ~ /\\[[:space:]]*$/) {
          incall = 1
...
for target in "${TARGETS[@]}"; do
  if [[ -d "$target" ]]; then
    ...
    # above EXPECTED_ZERO_FILES. KAN-490 widened it again to skills/flow-fast/'s
    # own phase files (review.md, finish.md); /flow-fast has since collapsed
    # into its single SKILL.md, which carries every one of its marks itself.
    FILES=()
  ...
  for f in "${FILES[@]:-}"; do
    ...
    while IFS=$'\t' read -r lineno verb cmd; do
      ...
      # `-harness` is a `stage begin` flag and `flow record dispatch` has
      ...
      # scanned call.
      has_harness=1
      if [[ "$verb" == "flow stage begin" ]] \
        && ! printf '%s' "$cmd" | grep -qE -- '(^|[[:space:]])-harness([[:space:]]|=)'; then
        has_harness=0
        printf '%s:%s: `flow stage begin` carries no -harness -- required so a recorded run states which harness marked it\n' "$f" "$lineno"
        VIOLATIONS=$((VIOLATIONS + 1))
      fi

      if [[ "$verb" == "flow stage begin" && "$has_harness" -eq 1 ]]; then
      ...
      guess=""
      [[ "$verb" == "flow stage begin" ]] && guess="$(guess_placeholder "$cmd")"
      if [[ -n "$guess" ]]; then
      ...
      if [[ "$verb" == "flow stage begin" ]]; then
        stage_key="$(stage_value "$cmd")"
        if [[ -z "$stage_key" ]]; then
