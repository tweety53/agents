...
real_home_fingerprint() {
  ...
  # The zcode layer also writes the user's shell rc files (install_zcode_env), so the
  # containment guarantee has to cover them too — a global run in a sandboxed HOME that
  # leaked into the real ~/.zshrc must fail this comparison.
  for p in "$REAL_HOME/.zshrc" "$REAL_HOME/.bashrc"; do
    if [[ -e "$p" ]]; then stat_line "$p"; else printf 'absent %s\n' "$p"; fi
  done
}
...
group "ZCode: global installs a self-contained ~/.zcode layer"

new_home; home="$HOME_DIR"
run_setup "$FIXTURE" "$home" global
assert_rc_zero "global with the zcode layer succeeds" "$RUN_RC" "$RUN_LOG"
assert_symlink "zcode skill links land beside the other harnesses'" "$home/.zcode/skills/demo-skill"
assert_symlink "zcode gets the claude command set, as the format is shared" "$home/.zcode/commands/demo.md"
assert_symlink "zcode rule full texts are linked" "$home/.zcode/rules/good-always.md"
assert_symlink "the zcode hook is linked" "$home/.zcode/hooks/enforce-agent-baseline.py"
assert_symlink "the zcode flow-active-change hook is linked" "$home/.zcode/hooks/flow-active-change.py"
assert_symlink "the zcode protect-main-checkout hook is linked" "$home/.zcode/hooks/protect-main-checkout.py"
assert_file_exists "the zcode agent baseline is generated, not linked" "$home/.zcode/rules/agent-baseline.md"
assert_contains "the generated baseline says where it came from" "$home/.zcode/rules/agent-baseline.md" "Generated by agents/setup.sh"
assert_contains "the zcode block points at zcode-local full texts" "$home/.zcode/AGENTS.md" "Full rule: \`~/.zcode/rules/cored.md\`"
...
new_home; home="$HOME_DIR"
run_setup "$FIXTURE" "$home" global
cp "$home/.zcode/AGENTS.md" "$SANDBOX/zcode-block-first.md"
run_setup "$FIXTURE" "$home" global
assert_rc_zero "the second global with the zcode layer succeeds" "$RUN_RC" "$RUN_LOG"
assert_identical "the zcode block is byte-identical on re-run" "$home/.zcode/AGENTS.md" "$SANDBOX/zcode-block-first.md"

group "ZCode: the per-project mode writes project paths only"

new_home; home="$HOME_DIR"
zproj="$SANDBOX/zcode-project"
run_setup "$FIXTURE" "$home" zcode "$zproj"
assert_rc_zero "the zcode project install succeeds" "$RUN_RC" "$RUN_LOG"
assert_symlink "project skills land in .zcode/skills" "$zproj/.zcode/skills/demo-skill"
assert_symlink "project commands land in .zcode/commands" "$zproj/.zcode/commands/demo.md"
assert_file_exists "AGENTS.md is copied to the project root" "$zproj/AGENTS.md"
assert_absent "no user-level zcode state is created" "$home/.zcode"
assert_absent "no claude state is created" "$home/.claude"

group "ZCode: the compact-window env block in the shell rc"

new_home; home="$HOME_DIR"
run_setup "$FIXTURE" "$home" global
assert_rc_zero "global with the env step succeeds" "$RUN_RC" "$RUN_LOG"
assert_contains ".zshrc carries the managed export" "$home/.zshrc" "export Z_COMPACT_WINDOW=500000"
assert_contains ".zshrc block is delimited" "$home/.zshrc" "# flow:zcode-env:begin"
assert_eq "exactly one begin marker" "1" "$(grep -cFx '# flow:zcode-env:begin' "$home/.zshrc")"
assert_absent "an absent .bashrc is not created" "$home/.bashrc"

# --- a pre-existing .bashrc gets the block too; re-run refreshes, never duplicates.
printf '# my bash notes\n' >"$home/.bashrc"
run_setup "$FIXTURE" "$home" global
assert_contains ".bashrc gets the block when it exists" "$home/.bashrc" "export Z_COMPACT_WINDOW=500000"
assert_contains "the user's own .bashrc content survives" "$home/.bashrc" "# my bash notes"
run_setup "$FIXTURE" "$home" global
assert_eq "re-run leaves exactly one block in .zshrc" "1" "$(grep -cFx '# flow:zcode-env:begin' "$home/.zshrc")"

# --- an unmanaged export wins: the installer warns and does not touch the file.
new_home; home="$HOME_DIR"
seed_file "$home/.zshrc" 644 <<'EOF'
# my own zsh setup
export Z_COMPACT_WINDOW=200000
EOF
cp -p "$home/.zshrc" "$SANDBOX/zshrc-unmanaged-expected.md"
run_setup "$FIXTURE" "$home" global
assert_identical "an unmanaged Z_COMPACT_WINDOW line is left byte-identical" "$home/.zshrc" "$SANDBOX/zshrc-unmanaged-expected.md"
assert_contains "the skip is explained in the log" "$RUN_LOG" "outside the managed block"
