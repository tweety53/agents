...
check_retired_stage_vocabulary() {
  ...
  # Drop this pattern's occurrence-level exemption (see "Legitimate exceptions" above) from each
  # candidate hit before deciding whether the line is real drift. Done here, once, on the assembled
  # "path:line:content" hits — not baked into $pattern itself — because the exemption is
  # per-occurrence, not per-line: every substring matching a backtick, `code-review`, a backtick,  # vocab-guard:allow
  # whitespace, and the word `skill` is removed.
  # The pattern is then re-applied to what remains. A hit that still matches after the removal
  # carries a genuine retired token — bare, backtick-fenced but followed by something other than  # vocab-guard:allow
  # `skill`, or any `code-review`-prefixed compound — and stands unchanged (the  # vocab-guard:allow
  # ORIGINAL content is reported, not the stripped version, so the printed hit still shows the
  # real line). A hit that matches nothing after the removal was drift-free and is dropped.
  if [[ -n "$hits" ]]; then
