...
new_fixture
mkdir -p "$FIXTURE/skills/flow-contracts"
printf '## Gone\n\nbody\n' > "$FIXTURE/rules/never-touch-production.mdc"
printf '**Do not** copy from `rules/never-touch-production.mdc` <!-- refs-guard:allow -->\n' \
  > "$FIXTURE/skills/flow-contracts/worktree-resolution-rationale.md"
run_guard "$FIXTURE"
...
# to a declared expected-zero member (skills/flow-contracts/worktree-resolution-rationale.md):
...
printf 'Follow **Panel re-runs** in `../flow-contracts/worktree-resolution-rationale.md`.\n' \
  > "$FIXTURE/skills/demo/SKILL.md"
run_guard "$FIXTURE"
...
# declared expected-zero member (skills/flow-contracts/worktree-resolution-rationale.md), as case
...
new_fixture
mkdir -p "$FIXTURE/skills/flow-contracts" "$FIXTURE/.flow"
