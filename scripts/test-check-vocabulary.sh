...
# ===========================================================================
# Case 6b: `code-review-low` -- a reviewer-slot id removed from the panel  # vocab-guard:allow
# (no longer in ValidReviewers, stats/internal/store/settings.go). It is
# retired vocabulary like the bare token it contains, so it is caught with no
# exemption, bare or backtick-fenced.
# ===========================================================================
new_fixture
printf 'Unreachable falls back to primary, principles, code-review-low.\n' > "$FIXTURE_FILE"  # vocab-guard:allow
run_guard "$FIXTURE"
[ "$RC" -eq 1 ] && pass "case 6b: retired slot id code-review-low caught" || fail "case 6b: rc=$RC out=$OUT"  # vocab-guard:allow

new_fixture
printf '`code-review-low` is one of the reviewer slots.\n' > "$FIXTURE_FILE"  # vocab-guard:allow
run_guard "$FIXTURE"
[ "$RC" -eq 1 ] && pass "case 6c: backtick-fenced code-review-low caught" || fail "case 6c: rc=$RC out=$OUT"  # vocab-guard:allow

# ===========================================================================
# Case 6d: a `code-review-`-prefixed compound -- `code-review-heavy` --  # vocab-guard:allow
# must still be caught. Regression case for a trailing  # vocab-guard:allow
