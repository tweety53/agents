...
# 26. KAN-439: the treeless-satellite fallback at the guard level. The
#     change directory exists but is EMPTY — no tasks.md, no link.md, the
#     KAN-423 shape no link mechanism can ever serve. With the canonical
#     worktree carrying a same-named plan, the verdict tracks that plan's
#     checkboxes; without the argument it is the ordinary missing-plan
#     verdict. Built from new_satellite_fixture with its link.md removed, so
#     the stub `flow` and the sat-demo naming match the satellite cases.

# 26a. Same-named canonical plan fully checked: CLEAR.
new_satellite_fixture peerx
rm "$WT/spectre/changes/sat-demo/link.md"
CANON26="$(mktemp -d "${TMPDIR:-/tmp}/unfinished-work-test.XXXXXX")"
SANDBOXES+=("$CANON26")
mkdir -p "$CANON26/spectre/changes/sat-demo"
printf -- '- [x] 1. done\n' > "$CANON26/spectre/changes/sat-demo/tasks.md"
run_guard "$WT" sat-demo "$CANON26"
assert_verdict "CLEAR:" "same-name canonical resolution resolves a treeless satellite"

# 26b. Same-named canonical plan with an unchecked item: OUTSTANDING against
#      that plan, never "no plan at".
new_satellite_fixture peerx
rm "$WT/spectre/changes/sat-demo/link.md"
CANON26B="$(mktemp -d "${TMPDIR:-/tmp}/unfinished-work-test.XXXXXX")"
SANDBOXES+=("$CANON26B")
mkdir -p "$CANON26B/spectre/changes/sat-demo"
printf -- '- [ ] 1. not done\n' > "$CANON26B/spectre/changes/sat-demo/tasks.md"
run_guard "$WT" sat-demo "$CANON26B"
assert_verdict "OUTSTANDING:" "a treeless satellite whose same-named canonical plan has an unchecked item is OUTSTANDING"
assert_reason "1 unchecked plan item(s)" "the treeless-satellite reason counts against the same-named canonical plan"
case "$OUT" in
  *"no plan at"*) fail "a same-name resolved plan must never be reported as \"no plan at\": $OUT" ;;
  *) pass "a same-name resolved plan is never reported as \"no plan at\"" ;;
esac

# 27. The same treeless shape with the canonical worktree OMITTED keeps the
#     ordinary missing-plan verdict: OUTSTANDING, "no plan at", exit 0.
new_satellite_fixture peerx
rm "$WT/spectre/changes/sat-demo/link.md"
run_guard "$WT" sat-demo
assert_verdict "OUTSTANDING:" "a treeless satellite with no canonical worktree argument is OUTSTANDING"
assert_reason "no plan at" "without the argument the treeless shape names the missing-plan signal"

# 28. A ## Part of link that names its plan and does not find it there is
#     NEVER rescued by same-name resolution, even when the canonical worktree
#     carries a same-named plan: exit 2, "cannot determine anything" — the
#     loud triage is unchanged (the lib harness's case 12, end to end).
new_satellite_fixture peerx
CANON28="$(mktemp -d "${TMPDIR:-/tmp}/unfinished-work-test.XXXXXX")"
SANDBOXES+=("$CANON28")
mkdir -p "$CANON28/spectre/changes/sat-demo"
printf -- '- [x] 1. done\n' > "$CANON28/spectre/changes/sat-demo/tasks.md"
run_guard "$WT" sat-demo "$CANON28"
[ "$RC" -eq 2 ] && pass "a failing Part of link with a same-named canonical plan still exits 2" \
  || fail "no-rescue case: expected exit 2, got rc=$RC out=$OUT"
case "$ERR" in
  *"cannot determine anything"*) pass "a failing Part of link with a same-named canonical plan says it cannot determine anything" ;;
  *) fail "no-rescue case: no such message on stderr: $ERR" ;;
esac

# 29. SIGNAL TWO READS FINDINGS VIA THE CANONICAL WORKTREE when the plan
#     resolved there. The stub records every findings call's arguments and
#     answers one OPEN finding: the plan signal is CLEAR (the same-named
#     canonical plan is fully checked), so the OUTSTANDING verdict can only
#     come from the finding — seen through the canonical -C, where the
#     change's records actually live, never through the satellite worktree's
#     own project key (which would answer a permissive []).
new_satellite_fixture peerx
rm "$WT/spectre/changes/sat-demo/link.md"
cat > "$WT/bin/flow" <<'STUB'
#!/usr/bin/env bash
...
chmod +x "$WT/bin/flow"
set_findings open
CANON29="$(mktemp -d "${TMPDIR:-/tmp}/unfinished-work-test.XXXXXX")"
SANDBOXES+=("$CANON29")
mkdir -p "$CANON29/spectre/changes/sat-demo"
printf -- '- [x] 1. done\n' > "$CANON29/spectre/changes/sat-demo/tasks.md"
run_guard "$WT" sat-demo "$CANON29"
assert_verdict "OUTSTANDING:" "signal two reads findings via the canonical worktree"
assert_reason "1 open finding(s) in the review panel record" "the open finding is counted through the canonical read"
grep -qF -- "-C $CANON29" "$WT/bin/findings.args" 2>/dev/null \
  && pass "the findings call ran with -C <canonical-worktree>" \
  || fail "the findings call never carried -C <canonical-worktree>: $(cat "$WT/bin/findings.args" 2>/dev/null)"
