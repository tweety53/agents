...
**When done:** Manual review (Gate B) again on the updated diff, or `/myflow-manual-test <name>` first if this was a Gate C fix — loop through `/myflow-do-fix` again as many times as needed. Once both gates are satisfied: `/myflow-review <name>`, then `/myflow-finish <name>` (also archives any nested `<name>-fix-N` sub-changes together). If this was a Gate D fix, ask the user to re-review the pushed commit on the PR and merge it (never merged by this command), then `/myflow-finish <name>` once merged.
