...
**Model:** keep this **session** on Sonnet (or your default). The **implementer subagents this command dispatches run on Opus** — or your harness's strongest model — named explicitly on each dispatch, never inherited; every review-panel slot stays on Sonnet. See "Model policy" in `skills/myflow-contracts/pipeline.md`, which is canonical. Cursor doesn't yet support a per-command model frontmatter field, so the session setting is a recommendation rather than an enforced switch — but the subagent models are set at dispatch time and apply in every harness.
