...
- **Run 1 — branch not merged.** Checks each worktree in the resolved set (see **Resolving a change's worktrees**, `skills/myflow-contracts/pipeline.md`) for **unfinished work** first — before the landing question and before any git action — and offers exactly three courses: stop and finish it first *(recommended)*, continue anyway, or file **or join** a Jira follow-up and then continue. Then asks how it should land: open a pull request (default), merge and push, or leave it to you. Makes **two commits** — the implementation first, the planning artifacts second — takes that route, moves the linked issue to **In Review** whichever route was taken, and **stops** — the state stays `IN_PROGRESS`.
