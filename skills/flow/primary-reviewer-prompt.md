...
Subagent (<the dispatch's subagent_type>):  # flow-review on `default`;
  ...
  prompt: |
    ...
    **Production readiness:**
    ...
    - Code review (low) covers the same code-quality ground on the rosters that carry
      it; raise a finding it would also raise only when it is a defect you would bet on,
      and never re-raise one its report already carries.
