...
1. Take the directory holding that `SKILL.md` — `.../skills/myflow-do/`, `.../skills/myflow-info/`,
...
This is the shape `myflow-info`'s own read falls back to, so a rule that depended on a link to
...
guess.** Check that the path you are about to use exists under it. A skill directory that was
**copied** into a project rather than linked resolves two levels up to something that is not the
agents repository — `.claude/`, or an unrelated project root — and there is no third step that
recovers the real one. That is the absent-script case, and each command says what it does with it;
it is never a licence to skip the step, and never a reason to search the filesystem for a checkout
that might be one.
...
**A dropped `url` row is not exempt from the refusal below.** The refusal is
