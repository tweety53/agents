...
- **Slot 2 (required)** — render with `[LENS]` = **Merged**. Runs on the **parent model**:
  **omit** the `model` parameter entirely.
- **Slots 5+ (conditional)** — render with `[LENS]` = **Lens B** or **Lens C**. Each of
  these **must** set `model` to `[ECONOMIC_MODEL_SLUG]` per the mapping in SKILL.md.
...
Subagent (generalPurpose):
  ...
  model: [ECONOMIC_MODEL_SLUG]            # slots 5+ ONLY — OMIT this line for slot 2
  prompt: |
    ...
    **If no standards files resolved** — the **Project standards** field above is
    ...
    - **Critical** — a hard-invariant violation (architecture/layer purity, a new
      ...
      retry path, a Least-Privilege widening. These block Gate B.
...
- `[PRINCIPLES_PATH]` — the **absolute** path of `engineering-principles.md` inside the
  ...
  `~/.claude/skills/openspec-apply-superpowers/engineering-principles.md`; under a
