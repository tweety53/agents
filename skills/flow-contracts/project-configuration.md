...
**`## planning model`'s body is matched exactly as `## default landing route`'s is (see above)** —
one `ValidModels` member in place of the three literals, and the same report-by-name-and-drop on a
body that does not match exactly one. `## self review model`'s body is matched exactly the same way.
`## self review`'s body is matched the same way — two literals in place of three, and the same
report-by-name-and-drop. `## execution mode`, `## implementer model` and `## review panel` are each
