...
`## self review`'s body is matched the same way — two literals in place of three, and the same
report-by-name-and-drop; `## handoff` likewise, `required` and `none` in place of three. `## execution mode`, `## implementer model` and `## review panel` are each
