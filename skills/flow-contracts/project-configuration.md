...
| `## implementer model` | Optional. One of the literal bodies `default` or `dynamic`, same shape as `## execution mode`. `default` runs the implementer on `DEFAULT_MODEL` at effort `default`; `dynamic` lets the plan's class decide both model and effort. Resolved into `IMPLEMENTER_MODEL_TOGGLE` (**Model resolution**, `skills/flow/SKILL.md`); absent, or a body matching neither literal exactly, is reported by name and dropped, resolving as `default`. |
