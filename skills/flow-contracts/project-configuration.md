...
| `## default landing route` | Optional. One of the literal bodies `pull request`, `merge and push` or `manual` — this section holds that value and nothing else, never free-form prose, matching `## jira`'s own single-line-literal shape. Used as `skills/flow/integrate.md`'s landing question's own `(default, recommended)` option; absent, or a body matching none of the three literals exactly, is reported by name and dropped, falling back to `pull request`. |
...
| `## self review` | Optional. One of the literal bodies `run`, `skip` or `defer` — this section holds that value and nothing else, never free-form prose, matching `## default landing route`'s own single-line-literal shape. Resolved by run 2's step 9 (`skills/flow/archive.md`) before its skip prompt: `skip` skips self-review without asking, `run` runs it without asking, `defer` saves the context bundle without asking (**Run 2 — the branch is merged**, `skills/flow-contracts/finish-contract-run2.md`, step 9) for `/flow-self-review` to consume, absent means the operator is asked. `/flow-fast` reads it too and honours `defer` alone (**5. Verify**, `skills/flow-fast/SKILL.md`). A body matching none of the three literals exactly is reported by name and dropped, resolving as if the key were absent. |
| `## self review model` | Optional. One member of `<agents repo>/stats/internal/store/settings.go`'s `ValidModels` map and nothing else, never free-form prose. When present and valid it wins over the settings store's `selfReviewModel` in `SELF_REVIEW_MODEL` resolution (step 9, `skills/flow/archive.md`); absent falls through to the store. A body matching no `ValidModels` member is reported by name (quoting what was found) and dropped, resolving as if the key were absent. |
| `## execution mode` | Optional. One of the literal bodies `default` or `dynamic` — this section holds that value and nothing else, never free-form prose, matching `## self review`'s own single-line-literal shape. `default` runs `sdd` with the parent dispatching an implementer per group; `dynamic` lets the plan's class decide. Resolved into `EXECUTION_MODE_TOGGLE` (**Model resolution**, `skills/flow/SKILL.md`); absent, or a body matching neither literal exactly, is reported by name and dropped, resolving as `default`. |
| `## implementer model` | Optional. One of the literal bodies `default` or `dynamic`, same shape as `## execution mode`. `default` runs the implementer on `DEFAULT_MODEL` at effort `default`; `dynamic` keeps `DEFAULT_MODEL` and lets the plan's class decide the effort. Resolved into `IMPLEMENTER_MODEL_TOGGLE` (**Model resolution**, `skills/flow/SKILL.md`); absent, or a body matching neither literal exactly, is reported by name and dropped, resolving as `default`. |
| `## review panel` | Optional. One of the literal bodies `default` or `dynamic`, same shape as `## execution mode`. `default` dispatches the settings-store roster on `DEFAULT_MODEL` with a delta rerun; `dynamic` lets the plan's class and rolls decide the roster. Resolved into `REVIEW_PANEL_TOGGLE` (**Model resolution**, `skills/flow/SKILL.md`); absent, or a body matching neither literal exactly, is reported by name and dropped, resolving as `default`. |
...
**`## self review model`'s body is matched exactly as `## default landing route`'s is (see
above)** — one `ValidModels` member in place of the three literals, and the same
report-by-name-and-drop on a body that does not match exactly one.
`## self review`'s body is matched the same way — `run`, `skip` and `defer` in place of the three
