...
**`STARTED`** — printed by `/flow`'s creating run, regenerated by `/flow-status <name>`
...
**`IN_PROGRESS`, after `/flow`'s implement phase** — printed by `/flow`'s implement phase, regenerated by `/flow-status <name>`
...
**`IN_PROGRESS`, after `/flow`'s integrate run** — printed by run 1, regenerated by `/flow-status <name>`
...
|--------------|---------|-----------|
| **no commits of its own** — `HEAD` is still the recorded merge base | either | `/flow`'s implement phase |
...
| inconclusive | none | `/flow`'s implement phase |
...
carries two more fields now: `**Self-review:** <path> (rating: <n>/5) | deferred —
