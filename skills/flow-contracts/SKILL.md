...
| [git-boundaries.md](git-boundaries.md) | Which git actions each command may take, and the guarded two-commit chain that enforces the split between implementation and planning artifacts. **Loaded by `/flow`'s implement phase, bare `/flow` and `/flow-fast`** |
