...
`skills/myflow-contracts/pipeline.md`, loaded on demand by `/flow`.
...
"Model policy" in `myflow-contracts/model-policy.md` for the per-harness enforcement notes that
still apply.
...
Every skill above but `flow-research` and `myflow-contracts` requires the `spectre` CLI
