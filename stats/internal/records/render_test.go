...
// TestRenderLedgerEndedAndDuration pins the kan-450 lines: a closed
// dispatch renders `- Ended:` (RFC3339 UTC) and `- Duration:` (rounded to
// the second) directly after `- Started:`, so a ledger reader sees how
// long the dispatch ran without querying the store.
func TestRenderLedgerEndedAndDuration(t *testing.T) {
	started := time.Date(2026, 1, 2, 3, 4, 5, 0, time.UTC)
	ended := started.Add(62 * time.Second)
	out := records.RenderLedger(records.Run{
		Change: "demo",
		Dispatches: []records.Dispatch{{
			Seq: 1, Role: "implementer", Model: "opus",
	...
	for _, want := range []string{
		...
		"- Ended: 2026-01-02T03:05:07Z\n",
		"- Duration: 1m2s\n",
	} {
		if !strings.Contains(out, want) {
			t.Errorf("RenderLedger output missing %q; got:\n%s", want, out)
		}
	}
}

// TestRenderLedgerOpenDispatchHasNoDuration pins the absence shapes: a
// still-open dispatch (EndedAt nil) renders `- Ended: not recorded` and no
// Duration line at all, and a dispatch with an end but no start renders
// the Ended line without a Duration -- a duration needs both instants, and
// one missing is an absence, never zero.
func TestRenderLedgerOpenDispatchHasNoDuration(t *testing.T) {
	open := records.RenderLedger(records.Run{
		Change: "demo",
		Dispatches: []records.Dispatch{{
			Seq:       1,
			Role:      "implementer",
	...
	if !strings.Contains(open, "- Ended: not recorded\n") {
		t.Errorf("open dispatch: missing \"- Ended: not recorded\"; got:\n%s", open)
	}
	if strings.Contains(open, "- Duration:") {
		t.Errorf("open dispatch: rendered a Duration line; got:\n%s", open)
	}

	ended := time.Date(2026, 1, 2, 3, 4, 5, 0, time.UTC)
	noStart := records.RenderLedger(records.Run{
		Change: "demo",
		Dispatches: []records.Dispatch{{
			Seq: 1, Role: "implementer", Model: "opus",
			EndedAt: &ended,
	...
	if !strings.Contains(noStart, "- Ended: 2026-01-02T03:04:05Z\n") {
		t.Errorf("no-start dispatch: missing Ended line; got:\n%s", noStart)
	}
	if strings.Contains(noStart, "- Duration:") {
		t.Errorf("no-start dispatch: rendered a Duration line; got:\n%s", noStart)
	}
}

// TestRenderLedgerCostSection pins the kan-450 totals section: a closing
// "## Cost by role and task" table with one row per (role, task) group, a
// Total row carrying the change's own figure, and the two absence counts.
func TestRenderLedgerCostSection(t *testing.T) {
	started := time.Date(2026, 1, 2, 3, 4, 5, 0, time.UTC)
	cost := 1.25
	out := records.RenderLedger(records.Run{
		Change: "demo",
		Dispatches: []records.Dispatch{
			{Seq: 1, Role: "implementer", TaskID: "1", Model: "opus", StartedAt: started,
				Metrics: json.RawMessage(`{"tokens":{"main":{"input":100,"output":20,"cache_read":5,"cache_creation":3},"sidechain":{"input":7,"output":1}}}`),
				CostUSD: &cost},
			{Seq: 2, Role: "reviewer", TaskID: "2", Model: "opus", StartedAt: started},
		},
	})
	for _, want := range []string{
		"## Cost by role and task\n",
		"| implementer | 1 | 1 | 107 | 21 | 5 | 3 | 1.25 |",
		"| reviewer | 2 | 1 | 0 | 0 | 0 | 0 | 0.00 |",
		"| Total | | 2 | 107 | 21 | 5 | 3 | 1.25 |",
...
// TestRenderLedgerCostSectionOmittedWithoutDispatches pins the empty
// ledger: no dispatches means the early return's "No dispatches" line and
// no totals section at all.
func TestRenderLedgerCostSectionOmittedWithoutDispatches(t *testing.T) {
	out := records.RenderLedger(records.Run{Change: "demo"})
	if strings.Contains(out, "Cost by role and task") {
		t.Errorf("empty ledger rendered a cost section; got:\n%s", out)
	}
	if !strings.Contains(out, "No dispatches were recorded for this change.") {
		t.Errorf("empty ledger missing its no-dispatches line; got:\n%s", out)
	}
}
