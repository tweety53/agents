...
// "verdict-false-positive"|"incident"|"substitution"|"decision"|"summary"|"pass"|"mutation",
// "request":<the
// records.Dispatch, records.DispatchEnd, records.Finding, status request,
// records.Verdict, records.VerdictFlag, records.Incident,
// records.Substitution, records.Decision, records.ChangeSummary,
// records.Pass or records.Mutation that was journalled>}. It is a
...
func (r *Reconciler) applyRecordEntry(ctx context.Context, e fallback.Entry) error {
	...
	case "summary":
		...
		if err := json.Unmarshal(body.Request, &in); err != nil {
			return fmt.Errorf("%w: decode change summary: %v", errRecordEntryDecodeFailed, err)
		}
		_, _, err := api.ApplyChangeSummaryRecord(ctx, r.recordStore, e.Project, e.Name, in.Summary)
		return err
	case "pass":
		var in records.Pass
		if err := json.Unmarshal(body.Request, &in); err != nil {
			return fmt.Errorf("%w: decode pass: %v", errRecordEntryDecodeFailed, err)
		}
		_, err := api.ApplyPassRecord(ctx, r.recordStore, e.Project, e.Name, in)
		return err
	case "mutation":
		var in records.Mutation
		if err := json.Unmarshal(body.Request, &in); err != nil {
			return fmt.Errorf("%w: decode mutation: %v", errRecordEntryDecodeFailed, err)
		}
		_, err := api.ApplyMutationRecord(ctx, r.recordStore, e.Project, e.Name, in)
		return err
	default:
...
func isDefinitiveRecordOutcome(err error) bool {
	...
	case errors.Is(err, store.ErrDeferredNotMinor):
		...
		// 409 for the identical body. The refusal keys on the row's
		// mutable severity, so a later severity change could flip a
		// future replay's answer -- but the live route refuses now, the
		// CLI journals the write whenever the store is down because
		// `record status` names no severity and cannot know the row's
		// own, and retiring now is the call the live route itself made.
		// Leaving it queued would block every valid entry behind it
		// until that severity changed, with nothing to bring the change
		// about -- the same no-progress stall ErrFindingNotFound above
		// retires.
		return true
