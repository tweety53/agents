...
// TestSetFindingStatusDeferredMinorOnly pins design.md's `deferred <reason>`
// section against the real store, not a hand-built mock: a Major finding is
// refused with ErrDeferredNotMinor and its status is left untouched, while a
// Minor finding is deferred successfully.
func TestSetFindingStatusDeferredMinorOnly(t *testing.T) {
	st, _ := newRecordStore(t)
	ctx := context.Background()
	projectKey := fmt.Sprintf("proj-record-status-deferred-%d", time.Now().UnixNano())
	seedChange(t, st, projectKey, "kan-1")

	major := baseFinding("F1", 0)
	major.Severity = "Major"
	if _, _, err := st.UpsertFinding(ctx, projectKey, "kan-1", major); err != nil {
	...
	for _, f := range rec.Findings {
		if f.Ref == "F1" && f.Status != "open" {
			t.Errorf("F1 status = %q after a refused deferral, want unchanged (open)", f.Status)
		}
	}
	...
	for _, f := range rec.Findings {
		if f.Ref == "F2" && f.Status != "deferred cosmetic" {
			t.Errorf("F2 status = %q, want %q", f.Status, "deferred cosmetic")
		}
	}
}

// TestSetFindingStatusDeferredAcceptsLowercaseMinor pins that the deferred
// guard matches severity case-insensitively, the same convention this
// package's Reviewers/Decisions queries already apply (severity is free
// text a caller writes verbatim -- records_test.go's own fixtures use both
// "important" and "Important"). A finding recorded with lowercase "minor"
// is genuinely Minor and must be deferrable, not refused with
// ErrDeferredNotMinor.
func TestSetFindingStatusDeferredAcceptsLowercaseMinor(t *testing.T) {
	st, _ := newRecordStore(t)
	ctx := context.Background()
	projectKey := fmt.Sprintf("proj-record-status-deferred-case-%d", time.Now().UnixNano())
	seedChange(t, st, projectKey, "kan-1")

	lower := baseFinding("F1", 0)
	lower.Severity = "minor"
	if _, _, err := st.UpsertFinding(ctx, projectKey, "kan-1", lower); err != nil {
		t.Fatalf("UpsertFinding F1: %v", err)
	}
	...
	for _, f := range rec.Findings {
		if f.Ref == "F1" && f.Status != "deferred cosmetic" {
			t.Errorf("F1 status = %q, want %q", f.Status, "deferred cosmetic")
		}
	}
}
...
// agent id and ended with none must still carry begin's id afterwards. See
// EndDispatch's own doc comment for why COALESCE, not a plain assignment,
// is required here -- a plain `SET agent_id = $8` would clear the column
// on every ordinary end call that omits it.
func TestEndDispatchOmittedAgentIDPreservesBeginsIdentifierAgainstPostgres(t *testing.T) {
	st, _ := newRecordStore(t)
	ctx := context.Background()
	projectKey := fmt.Sprintf("proj-end-dispatch-agent-id-%d", time.Now().UnixNano())
	seedChange(t, st, projectKey, "kan-1")

	const (
		sessionID = "session-end-dispatch-agent-id"
	...
	dispatch := baseDispatch("implementer", "opus")
	dispatch.SessionToken = token
	dispatch.Key = key
	dispatch.AgentID = "agent-begin-original"
	if _, err := st.RecordDispatch(ctx, projectKey, "kan-1", dispatch); err != nil {
		t.Fatalf("RecordDispatch: %v", err)
	}

	out, err := st.EndDispatch(ctx, projectKey, "kan-1", records.DispatchEnd{
		SessionToken: token,
		Key:          key,
		Outcome:      "completed",
		EndedAt:      dispatch.StartedAt.Add(time.Minute),
		// AgentID deliberately omitted -- this is the case COALESCE exists
		// for: an end that says nothing about the identifier must leave
		// begin's already-recorded one untouched.
	})
	if err != nil {
		t.Fatalf("EndDispatch: %v", err)
	}
	if out.AgentID != "agent-begin-original" {
		t.Errorf("EndDispatch's own returned row: AgentID = %q, want agent-begin-original -- an omitted end value must not clear what begin recorded", out.AgentID)
	}
	...
	if rec.Dispatches[0].AgentID != "agent-begin-original" {
		t.Errorf("stored row: AgentID = %q, want agent-begin-original -- re-read from Postgres after the update, not just the call's own return value", rec.Dispatches[0].AgentID)
	}
}
...
// TestMarkDispatchesUnattributed pins the dispatch-grain counterpart of a
// persisted give-up: every dispatch carrying the given session token has
// the reason merged into its metrics bag, a dispatch under a different
// token is untouched, and a dispatch that already carries a real tokens
// figure keeps it -- a dispatch that was measured and then had its session
// marked given up is a contradiction the store must not resolve by
// destroying the measurement.
func TestMarkDispatchesUnattributed(t *testing.T) {
	st, _ := newRecordStore(t)
	ctx := context.Background()
	projectKey := fmt.Sprintf("proj-unattributed-%d", time.Now().UnixNano())
	seedChange(t, st, projectKey, "kan-1")

	const (
		tokenA = "mf-unattributed-a"
		tokenB = "mf-unattributed-b"
		reason = "session never bound"
	)

	unmeasured := baseDispatch("implementer", "opus")
	unmeasured.SessionToken = tokenA
	...
	untouched.SessionToken = tokenB
	untouchedRecorded, err := st.RecordDispatch(ctx, projectKey, "kan-1", untouched)
	if err != nil {
		t.Fatalf("RecordDispatch untouched: %v", err)
	}

	if err := st.MarkDispatchesUnattributed(ctx, tokenA, reason, 0); err != nil {
		t.Fatalf("MarkDispatchesUnattributed: %v", err)
	}
	...
	if want := json.RawMessage(`{"unattributed":{"reason":"session never bound"}}`); !jsonEqual(t, byID[unmeasuredRecorded.ID], want) {
		t.Errorf("unmeasured dispatch metrics = %s, want %s -- candidates is omitted for a non-ambiguity reason", byID[unmeasuredRecorded.ID], want)
	}
	...
	if want := json.RawMessage(`{}`); !jsonEqual(t, byID[untouchedRecorded.ID], want) {
		t.Errorf("dispatch under a different token's metrics = %s, want %s -- the stamp must not reach a dispatch outside the given token", byID[untouchedRecorded.ID], want)
	}
}

// TestMarkDispatchesUnattributedRepeatStampIsIdempotent pins the repair for
// F18 (review panel round 1): unattributed.candidates is a snapshot fact,
// not a delta, so restamping the same ambiguity must never inflate it.
// attributeDispatches restamps on every harvest cycle that still sees the
// same unresolved ambiguity, and a give-up retried after a restart restamps
// too -- both are ordinary, repeated calls with the identical reason and
// candidates count, not a sign of a second, independent ambiguity. The
// stamp must therefore replace the "unattributed" key wholesale rather than
// summing its candidates leaf.
func TestMarkDispatchesUnattributedRepeatStampIsIdempotent(t *testing.T) {
	st, _ := newRecordStore(t)
	ctx := context.Background()
	projectKey := fmt.Sprintf("proj-unattributed-repeat-%d", time.Now().UnixNano())
	seedChange(t, st, projectKey, "kan-1")

	const (
		token  = "mf-unattributed-repeat"
	...
	// Two identical stamps, as a session that stays ambiguous across two
	// harvest cycles produces.
	if err := st.MarkDispatchesUnattributed(ctx, token, reason, 2); err != nil {
		t.Fatalf("MarkDispatchesUnattributed (1st): %v", err)
	}
	if err := st.MarkDispatchesUnattributed(ctx, token, reason, 2); err != nil {
		t.Fatalf("MarkDispatchesUnattributed (2nd): %v", err)
	}
	...
	// A third stamp with a different reason and candidates count -- as a
	// dispatch reattributed from one ambiguity into a different one would
	// produce -- must replace the stale reason and count wholesale, not
	// merge them.
	if err := st.MarkDispatchesUnattributed(ctx, token, "session never bound", 0); err != nil {
		t.Fatalf("MarkDispatchesUnattributed (reason change): %v", err)
	}
...
// TestRecordDecisionUpsertsPerSession asserts (change_id, session_token) is
// the upsert key design.md's decisions-jsonb-row decision names: recording
// twice under the same token leaves exactly one row, and the second call's
// body is the one that survives -- the same "second write wins" shape
// UpsertFinding gives a ref.
func TestRecordDecisionUpsertsPerSession(t *testing.T) {
	st, pool := newRecordStore(t)
	ctx := context.Background()
	projectKey := fmt.Sprintf("proj-decision-upsert-%d", time.Now().UnixNano())
	seedChange(t, st, projectKey, "kan-1")

	first, created, err := st.RecordDecision(ctx, projectKey, "kan-1", records.Decision{
		SessionToken: "sess-1",
		Decision:     json.RawMessage(`{"class":"small"}`),
	})
	if err != nil {
		t.Fatalf("first RecordDecision: %v", err)
	}
	if !created {
		t.Errorf("first RecordDecision created = false, want true")
	}

	second, created, err := st.RecordDecision(ctx, projectKey, "kan-1", records.Decision{
		SessionToken: "sess-1",
		Decision:     json.RawMessage(`{"class":"regular"}`),
	})
	if err != nil {
		t.Fatalf("second RecordDecision: %v", err)
	}
	if created {
		t.Errorf("second RecordDecision created = true, want false (same session token)")
	}
	if second.ID != first.ID {
		t.Errorf("second RecordDecision ID = %d, want %d (same row, updated)", second.ID, first.ID)
	}
	var gotClass struct {
		Class string `json:"class"`
	}
	if err := json.Unmarshal(second.Decision, &gotClass); err != nil {
		t.Fatalf("unmarshal second RecordDecision Decision: %v", err)
	}
	if gotClass.Class != "regular" {
		t.Errorf("second RecordDecision Decision class = %q, want %q (the second call's body to win)", gotClass.Class, "regular")
	}

	var count int
	if err := pool.QueryRow(ctx, "SELECT count(*) FROM decisions WHERE session_token = 'sess-1'").Scan(&count); err != nil {
		t.Fatalf("count decisions rows: %v", err)
	}
	if count != 1 {
		t.Errorf("decisions holds %d rows for sess-1, want exactly 1", count)
	}

	if _, _, err := st.RecordDecision(ctx, projectKey, "kan-does-not-exist", records.Decision{
		SessionToken: "sess-2", Decision: json.RawMessage(`{}`),
	}); !errors.Is(err, store.ErrChangeNotFound) {
		t.Errorf("RecordDecision with an unknown change error = %v, want errors.Is(_, store.ErrChangeNotFound)", err)
	}

	if _, _, err := st.RecordDecision(ctx, projectKey, "kan-1", records.Decision{
		SessionToken: "", Decision: json.RawMessage(`{}`),
	}); !errors.Is(err, store.ErrInvalidDecision) {
		t.Errorf("RecordDecision with an empty session token error = %v, want errors.Is(_, store.ErrInvalidDecision)", err)
	}

	if _, _, err := st.RecordDecision(ctx, projectKey, "kan-1", records.Decision{
		SessionToken: "sess-3", Decision: json.RawMessage(`not json`),
	}); !errors.Is(err, store.ErrInvalidDecision) {
		t.Errorf("RecordDecision with invalid JSON error = %v, want errors.Is(_, store.ErrInvalidDecision)", err)
	}
}

// TestListDecisionsNewestFirst asserts ListDecisions orders newest first
// and that the project filter excludes another project's rows -- the same
// shape TestListIncidentsNewestFirst asserts for incidents.
func TestListDecisionsNewestFirst(t *testing.T) {
	st, _ := newRecordStore(t)
	ctx := context.Background()
	projectKey := fmt.Sprintf("proj-decision-list-%d", time.Now().UnixNano())
	otherProjectKey := fmt.Sprintf("proj-decision-list-other-%d", time.Now().UnixNano())
	seedChange(t, st, projectKey, "kan-1")
	seedChange(t, st, otherProjectKey, "kan-2")

	older, _, err := st.RecordDecision(ctx, projectKey, "kan-1", records.Decision{
		SessionToken: "sess-older", Decision: json.RawMessage(`{"n":1}`),
	})
	if err != nil {
		t.Fatalf("RecordDecision older: %v", err)
	}
	time.Sleep(2 * time.Millisecond)
	newer, _, err := st.RecordDecision(ctx, projectKey, "kan-1", records.Decision{
		SessionToken: "sess-newer", Decision: json.RawMessage(`{"n":2}`),
	})
	if err != nil {
		t.Fatalf("RecordDecision newer: %v", err)
	}
	if _, _, err := st.RecordDecision(ctx, otherProjectKey, "kan-2", records.Decision{
		SessionToken: "sess-other", Decision: json.RawMessage(`{"n":3}`),
	}); err != nil {
		t.Fatalf("RecordDecision other project: %v", err)
	}

	list, err := st.ListDecisions(ctx, projectKey, "kan-1")
	if err != nil {
		t.Fatalf("ListDecisions: %v", err)
	}
	if len(list) != 2 {
		t.Fatalf("ListDecisions returned %d rows, want 2 (the other project's row must be excluded)", len(list))
	}
	if list[0].ID != newer.ID || list[1].ID != older.ID {
		t.Errorf("ListDecisions = [%d, %d], want newest first [%d, %d]", list[0].ID, list[1].ID, newer.ID, older.ID)
	}
}

// TestRecordDispatchStoresEffort pins design.md's Enforcement rule that an
// absent effort is a known fact -- the dispatcher set none -- never an
// unknown one: a dispatch recorded with no Effort must read back "default"
// rather than an empty string or NULL, and one recorded with an explicit
// effort must read back exactly that word.
func TestRecordDispatchStoresEffort(t *testing.T) {
	st, _ := newRecordStore(t)
	ctx := context.Background()
	projectKey := fmt.Sprintf("proj-record-effort-%d", time.Now().UnixNano())
	seedChange(t, st, projectKey, "kan-1")

	withNoEffort := baseDispatch("implementer", "opus")
	stored, err := st.RecordDispatch(ctx, projectKey, "kan-1", withNoEffort)
	if err != nil {
		t.Fatalf("RecordDispatch (no effort): %v", err)
	}
	if stored.Effort != "default" {
		t.Errorf("RecordDispatch's own return: Effort = %q, want %q for a dispatch recorded with none", stored.Effort, "default")
	}

	withEffort := baseDispatch("reviewer", "sonnet")
	withEffort.Effort = "high"
	if _, err := st.RecordDispatch(ctx, projectKey, "kan-1", withEffort); err != nil {
		t.Fatalf("RecordDispatch (effort=high): %v", err)
	}
	...
	if rec.Dispatches[0].Effort != "default" {
		t.Errorf("stored row 1: Effort = %q, want %q -- re-read from Postgres, not just RecordDispatch's own return", rec.Dispatches[0].Effort, "default")
	}
	if rec.Dispatches[1].Effort != "high" {
