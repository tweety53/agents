...
-- the empty string" -- both already mean the same thing here, the literal
-- "fable". Unlike default_model, an empty self_review_model is a valid,
-- meaningful value (fable) rather than a missing one, so the default
-- itself -- not just NOT NULL -- carries that meaning: a fresh row
-- created before /flow-settings has ever set this column already resolves
-- to "fable" rather than an unset state a caller must special-case.
