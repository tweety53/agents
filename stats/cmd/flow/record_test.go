...
// TestValidateFindingReproducerRejectsMetacharacters pins KAN-382: panel
// slots recorded grep/sed pipelines whose pipes and quotes the read-time
// guard rejects only after the finding has landed in the store. Every
// character of the mirrored set must refuse on its own, so no single
// character can regress without its own failure.
func TestValidateFindingReproducerRejectsMetacharacters(t *testing.T) {
	for i, mc := range reproducerMetachars {
		reproducer := "scripts/repro.sh" + string(mc) + "arg"
		if err := validateFindingReproducer(reproducer); err == nil {
			t.Fatalf("validateFindingReproducer(%q) = nil, want an error -- metachar %q (set index %d)", reproducer, string(mc), i)
		}
	}
}

// TestValidateFindingReproducerRejectsLeadingDashPathToken mirrors the
// read-time guard's rule that a runnable reproducer names a path, never an
// option, bound to the first whitespace-separated token alone.
func TestValidateFindingReproducerRejectsLeadingDashPathToken(t *testing.T) {
	if err := validateFindingReproducer("-scripts/repro.sh"); err == nil {
		t.Fatal(`validateFindingReproducer("-scripts/repro.sh") = nil, want an error -- a leading '-' on the path token looks like an option to whatever runs it`)
	}
	if err := validateFindingReproducer("scripts/repro.sh --strict"); err != nil {
		t.Fatalf("validateFindingReproducer(%q) = %v, want nil -- the dash rule binds to the path token alone", "scripts/repro.sh --strict", err)
	}
}

// TestValidateFindingReproducerRejectsURL mirrors the read-time guard's rule
// that a reproducer names a path inside the worktree, never a network
// location.
func TestValidateFindingReproducerRejectsURL(t *testing.T) {
	for _, reproducer := range []string{"http://evil.example/x", "scripts/repro.sh http://evil.example/x"} {
		if err := validateFindingReproducer(reproducer); err == nil {
			t.Fatalf("validateFindingReproducer(%q) = nil, want an error -- '://' names a network location", reproducer)
		}
	}
}

// TestValidateFindingReproducerRejectsAbsolutePathToken mirrors the
// read-time guard's lexical containment: an absolute token names a location
// outside every worktree, on the path token or on any argument.
func TestValidateFindingReproducerRejectsAbsolutePathToken(t *testing.T) {
	for _, reproducer := range []string{"/etc/passwd", "scripts/repro.sh /tmp/evil"} {
		if err := validateFindingReproducer(reproducer); err == nil {
			t.Fatalf("validateFindingReproducer(%q) = nil, want an error -- an absolute token cannot be contained in any worktree", reproducer)
		}
	}
}

// TestValidateFindingReproducerRejectsDotDotSegment mirrors the read-time
// guard's segment check: '..' as a whole token, a leading one, a trailing
// one and a mid-path one all walk out of the worktree. foo..bar.sh is not a
// segment and must stay legal.
func TestValidateFindingReproducerRejectsDotDotSegment(t *testing.T) {
	for _, reproducer := range []string{"../scripts/repro.sh", "scripts/../../repro.sh", "scripts/repro.sh ../../evil"} {
		if err := validateFindingReproducer(reproducer); err == nil {
			t.Fatalf("validateFindingReproducer(%q) = nil, want an error -- a '..' path segment walks out of the worktree", reproducer)
		}
	}
	if err := validateFindingReproducer("scripts/foo..bar.sh"); err != nil {
		t.Fatalf("validateFindingReproducer(%q) = %v, want nil -- '..' there is not a path segment", "scripts/foo..bar.sh", err)
	}
}

// TestValidateFindingReproducerRejectsHyphenInsteadOfEmDash pins KAN-382's
// nonstandard exemption forms: a hyphen where the em dash belongs makes the
// value a first-word-"none" value that is not the exemption, which record.go
// has always refused -- this test keeps that refusal visible beside the new
// shape rules.
func TestValidateFindingReproducerRejectsHyphenInsteadOfEmDash(t *testing.T) {
	if err := validateFindingReproducer("none - cannot reproduce"); err == nil {
		t.Fatal(`validateFindingReproducer("none - cannot reproduce") = nil, want an error -- the exemption form is 'none — <reason>' with an em dash`)
	}
}

// TestValidateFindingReproducerRejectsSpaceIndentedExemptionReason mirrors
// the read-time guard byte for byte: 'none — ' must be followed by a
// non-space character, so a doubled space before the reason is the bare-none
// refusal, not a legal exemption.
func TestValidateFindingReproducerRejectsSpaceIndentedExemptionReason(t *testing.T) {
	if err := validateFindingReproducer("none —  cannot reproduce"); err == nil {
		t.Fatal(`validateFindingReproducer("none —  cannot reproduce") = nil, want an error -- the reason must start immediately after 'none — '`)
	}
}

// TestValidateFindingReproducerAcceptsPlainPathWithArguments pins the
// accepted shape: a bare relative path, optionally followed by plain
// arguments, and the exemption form itself. The semicolon-carrying exemption
// reason pins F1 (mutation slot, round 0): the exemption path short-circuits
// the command checks exactly as the read-time guard's exemption does, so a
// reason may carry a metacharacter that a command may not.
func TestValidateFindingReproducerAcceptsPlainPathWithArguments(t *testing.T) {
	for _, reproducer := range []string{
		"scripts/repro.sh",
		"scripts/repro.sh arg1 arg2",
		".superpowers/sdd/reproducers/0-bugbot-1.sh",
		"none — no in-repo reproduction is possible",
		"none — cannot demo; see the runner notes",
	} {
		if err := validateFindingReproducer(reproducer); err != nil {
			t.Fatalf("validateFindingReproducer(%q) = %v, want nil", reproducer, err)
		}
	}
}

// TestReproducerMetacharSetSyncsWithBashSource pins KAN-382's
// reproducer-sourcing decision: scripts/reproducer-metachars.sh is the single
// bash source of truth for the banned set, and its header records two real
// drifts between its two bash consumers before this Go mirror existed. The
// test parses the assignment and fails on any drift, so a character added to
// one side without the other fails the build instead of silently narrowing
// one consumer's check.
func TestReproducerMetacharSetSyncsWithBashSource(t *testing.T) {
	raw, err := os.ReadFile("../../../scripts/reproducer-metachars.sh")
	if err != nil {
		t.Fatalf("read scripts/reproducer-metachars.sh: %v", err)
	}
	var line string
	for _, l := range strings.Split(string(raw), "\n") {
		if strings.HasPrefix(strings.TrimLeft(l, " \t"), "REPRODUCER_METACHARS=") {
			line = l
			break
		}
	}
	if line == "" {
		t.Fatal("no REPRODUCER_METACHARS= line in scripts/reproducer-metachars.sh")
	}
	got, ok := parseBashSingleQuotedAssignment(line)
	if !ok {
		t.Fatalf("could not parse the REPRODUCER_METACHARS assignment from %q", line)
	}
	if got != reproducerMetachars {
		t.Fatalf("REPRODUCER_METACHARS in scripts/reproducer-metachars.sh = %q, Go reproducerMetachars = %q -- the set drifted; update both in one change", got, reproducerMetachars)
	}
}

// parseBashSingleQuotedAssignment reads a bash assignment whose value is one
// single-quoted word carrying the close-escape-reopen idiom for an embedded
// single quote, returning the literal value. Only the shapes
// reproducer-metachars.sh actually uses are handled: an opening quote,
// literal segments, the embedded-quote continuation, and the closing quote.
func parseBashSingleQuotedAssignment(line string) (string, bool) {
